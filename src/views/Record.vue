<template>
  ...
  <div class="container">
    <div class="question" v-for="(question,k) in questions">
      ...
      <div v-else>
        <div class="choice" v-if="question.choice_a">A、{{ question.choice_a }}</div>
        <div class="choice" v-if="question.choice_b">B、{{ question.choice_b }}</div>
        <div class="choice" v-if="question.choice_c">C、{{ question.choice_c }}</div>
        <div class="choice" v-if="question.choice_d">D、{{ question.choice_d }}</div>
        <div style="display: flex;line-height: 40px;justify-content: left">
          <div class="answer">正确答案：{{ question.right_answer }}</div>
          <div class="answer" :class="{wrong:question.right_answer!==question.answer}">
            我的答案：{{ question.answer || "未作答" }}
